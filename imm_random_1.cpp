...
struct Parameters
{
    ...
    {
        assert (arr_period > 0);
        assert ((res_period > 0) && (res_period <= arr_period)); // ERROR
        assert (arr_period <= delay_bound);
        assert (delay_bound > 0);
        assert (window_size >= 0);
        assert ((det_per >= 0) && (det_per <= 1)); // ERROR
        assert ((ran_per >= 0) && (ran_per <= 1)); // ERROR
        assert (seed >= 0); // ERROR
...
std::ostream& operator<< (std::ostream& output, const Parameters& params)
{
    output << "res_period " << params.res_period << std::endl
                  ...
                  << "h_init " << params.current_age << std::endl
                  << "m_init " << params.current_size << std::endl
                  << "n_init " << params.batch_index << std::endl
...
int main(int argc, char** argv){
    ...
    int start_batch_index = batch_index;
    ...
    params.current_size = current_size;
