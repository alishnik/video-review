...
double success(double q){
    return ((double) rand() / (RAND_MAX)) > q; //ѕодбрасывание монетки, где q --- веро€тность неудачи
}
...
int main(int argc, char** argv){
    ...
    params.CheckValues(); // ѕроверка корректности
    ...
    while(true){
        Event tmp = events.top();        //“екущее событие
        if (tmp.time > simtime)            //—обытие наступает за пределами окна
            break;
        current_time = tmp.time;        //“екущее врем€, на выходе из цикла будет записано врем€ последнего событи€
        ...
        //FOR DEBUG
        std::cout << "dropped: " << dPackets << "\tSuccess:\t" << sPackets << "\trandSuccess:\t" << srandPackets << std::endl << std::endl;
    }
